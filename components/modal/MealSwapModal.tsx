...
interface Meal {
  ...
  description: string;
  price: number;
  image?: string; // Optional image field for future use
}
...
const MealSwapModal: React.FC<MealSwapModalProps> = ({
  ...
  // Fetch available meals when modal opens
  useEffect(() => {
    if (!isOpen) return;
    ...
    const fetchMeals = async () => {
      try {
        ...
        setFilteredMeals(response.availableMeals); // Initialize filtered list
      } catch (err) {
        console.log(err);
      }
    };
  ...
  const columns = [
    {
      ...
      render: (_: any, record: Meal) => (
        <div className="flex items-center space-x-4">
          {/* Placeholder for image; update if API provides image URLs */}
      ...
      <div className="bg-white rounded-xl w-full max-w-md md:max-w-lg lg:max-w-2xl max-h-[90vh] flex flex-col relative">
        ...
        <div className="sticky top-0 z-10 bg-white p-4 border-b border-gray-200">
          <div className="flex justify-between items-center mb-4">
            ...
            <button
              className="text-gray-400 hover:text-gray-600 p-2 bg-white rounded-full shadow-sm"
              onClick={onClose}
            >
              <svg
                className="h-5 w-5"
        ...
        <div className="flex-1 overflow-y-auto p-4">
          ...
          {!loading && !error && filteredMeals.length > 0 && (
            <Table
              ...
              rowKey="name" // Assuming name is unique; adjust if necessary
              pagination={false} // Disable pagination for simplicity; enable if needed
              onRow={(record) => ({
                onClick: () => onSelectMeal(record), // Select meal on row click
