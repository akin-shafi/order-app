...
const WeeklyMealPlanModal: React.FC<WeeklyMealPlanModalProps> = ({
  ...
  const handleGenerateMealPlan = async () => {
    ...
    const response = await generateMealPlan({ startDate: startDate.toISOString(), deliveryAddress });
    if (response) {
                  ...
                  {!mealPlan && (
                    <button
                      className={`w-full py-3 rounded-lg text-white ${
                        startDate && deliveryAddress ? "bg-[#FF6600]" : "bg-gray-300 cursor-not-allowed"
                      }`}
                      ...
                      {(activeTab === "Breakfast" ? mealPlan.breakfast : mealPlan.lunch).map((dailyMeal, index) => (
                        <div key={index} className="border border-gray-200 rounded-lg p-4">
                          <div className="flex justify-between items-center mb-2">
                            <span className="text-sm font-semibold text-[#292d32]">{dailyMeal.day}</span>
                            <button
                          ...
                          <div>
                            <p className="text-sm text-[#292d32]">{dailyMeal.meal.name}</p>
                            <p className="text-sm text-gray-500">{dailyMeal.meal.description}</p>
                            <p className="text-sm text-[#292d32] mt-1">₦{dailyMeal.meal.price.toLocaleString()}</p>
                          </div>
                        </div>
                      ))}
                    </div>
